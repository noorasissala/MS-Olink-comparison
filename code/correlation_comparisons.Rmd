...
- "Multi-platform proteomic analysis of Alzheimer’s disease cerebrospinal fluid and plasma reveals network biomarkers associated with proteostasis and the matrisome" (Dammer et al., 2022) (MS, Olink, and SomaScan data)
- "The Current Landscape of Plasma Proteomics: Technical Advances, Biological Insights, and Biomarker Discovery" (Kirsher et al., 2025). (MS, Olink, and SomaScan data)
...
- "Comparative studies of 2168 plasma proteins measured by two affinity-based platforms in 4000 Chinese adults" (Wang et al., 2025)
- "Cross-Ancestry Comparison of Aptamer and Antibody Proteomics Measures" (Nicholas et al., 2025, preprint)
- "Correlation between Olink and SomaScan proteomics platforms in adults with a Fontan circulation" (Assi et al., 2025)
...
rooney2025_cors <- read_xlsx(
  ...
  dplyr::rename(UniProt = UniprotID.Soma.11k,
                ...
                SomaScan.SeqID = SeqID) |> 
  mutate(N_Previous.study = 102) # values <LOD were retained, so there were no missing values and N is 102 for all proteins)
...
kirsher_cors <- read_xlsx(
  'data/external_data/42004_2025_1665_MOESM4_ESM.xlsx',
  sheet = 'SD 3- Interplatform Correlation', 
  .name_repair = 'universal_quiet',
  skip = 1) |> 
...
# Wang et al.
wang_cors <- read_xlsx(
  'data/external_data/41467_2025_56935_MOESM4_ESM.xlsx', 
  sheet = 'Supplementary_Data_1', .name_repair = 'universal_quiet') |> 
  dplyr::rename(UniProt = uniprot_id,
                Correlation_Previous.study = rho_olink_soma_ANML,
                SomaScan.SeqID = somascan_id,
                OlinkID = olink_id) |> 
  mutate(N_Previous.study = 3976)  # assuming that values <LOD were retained, there were no missing values, so N is 3976 for all proteins

# Nicholas et al.
nicholas_cors <- read_xlsx(
  'data/external_data/9aa3a2cf7b7a21eb1aa9c0bc.xlsx', 
  sheet = 'ST2.Correlation_Per_Probe', skip = 2, .name_repair = 'universal_quiet') |> 
  dplyr::rename(Correlation_Previous.study = rho_all_SMP_normalized,
                SomaScan.SeqID = SeqId) |> 
  mutate(N_Previous.study = 1930)  # assuming that values <LOD were retained, there were no missing values, so N is 1930 for all proteins

# Assi et al.
assi_cors <- read_xlsx(
  'data/external_data/1-s2.0-S2666668525000205-mmc2.xlsx', 
  sheet = 'Supplemental Data 1', skip = 4, .name_repair = 'universal_quiet') |> 
  dplyr::rename(Correlation_Previous.study = Rho,
                P.value_Previous.study = p,
                SomaScan.SeqID = AptName) |> 
  mutate(N_Previous.study = 71)  # assuming that values <LOD were retained, there were no missing values, so N is 130 for all proteins
...
cor_tables <- list(
  ...
  'Wang et al.' = wang_cors,
  'Nicholas et al.' = nicholas_cors,
  'Assi et al.' = assi_cors) |> 
  map(\(x) dplyr::select(
...
comparisons <- tibble(
  ...
  Comparison = case_when(
    ...
    Study %in% c('Dammer et al. (Olink-SomaScan)', 'Katz et al.', 'Eldjarn et al.',
                 ...
                 'Kirsher et al. (Olink 5K - SomaScan 11K)', 
                 'Wang et al.', 'Nicholas et al.', 'Assi et al.') ~ 'Olink-SomaScan'
  )
)
...
cor_table <- cor_tables |>
  ...
  mutate(Cor.interval_Previous.study = cor_intervals(Correlation_Previous.study),
         Cor.method_Previous.study = ifelse(
           str_detect(Study, 'Dammer|Nicholas'), 'Pearson', 'Spearman')) |>
...
cor_table_summarized <- cor_table |> 
  ...
  bind_rows(filter(
    cor_table[c('Study', 'Comparison', 'UniProt', 'Gene.Name', 'Correlation_Previous.study', 'Correlation_Sissala')], 
    Study == 'Finkernagel et al.')) # put back Finkernagel et al. correlations
...
study_summary <- study_summary |> 
  ...
  dplyr::relocate(Overlap.with.present.study, .before = Median.correlation_Overlap)
...
comparison_histogram <- function(cor_table, comparison, ncol = 2, nrow = NULL,
                                   facet_labels = NULL, pval_table = NULL) {
  ...
  if (is.null(pval_table)) {
    ...
    pval_df <- facet_data %>%
      ...
      rowwise() |>
      mutate(pval_label = format_pval_label(p.adj)) |> 
      ungroup()
    
  } else {
    pval_df <- pval_table %>%
      dplyr::select(Study, p.adj) %>%
      mutate(
        npcx = "left",
        npcy = 0.8) |> 
      rowwise() |>
      mutate(pval_label = format_pval_label(p.adj)) |> 
      ungroup() |> 
      filter(Study %in% unique(facet_data$Study))
  }
  
  # Default empty named vector if facet_labels is NULL
...
ms_soma_cors <- cor_table |> 
  ...
  mutate(Study = ifelse(str_detect(Study, 'Dammer'),
                       'Dammer et al.', Study)) |> 
  dplyr::select(-OlinkID_Previous.study)
...
# Calculate and adjust p-values for Wilcoxon tests comparing correlations in the present study and previous studies
olink_soma_pvals <- cor_table_summarized |> 
  filter(Comparison == 'Olink-SomaScan', !is.na(Correlation_Previous.study),
         !is.na(Correlation_Sissala)) |> 
  group_by(Study) |> 
  summarise(p.value = wilcox.test(Correlation_Previous.study, Correlation_Sissala)$p.value,
            .groups = 'drop') |> 
  mutate(p.adj = p.adjust(p.value, method = 'fdr'))

# Plot histograms in two pages
cor_table_summarized |> 
  filter(Study %in% olink_soma_studies[1:8]) |> 
  comparison_histogram(comparison = 'Olink-SomaScan', nrow = 4, ncol = 2,
                       facet_labels = facet_labels, pval_table = olink_soma_pvals) +
...
cor_table_summarized |> 
  ...
  comparison_histogram(comparison = 'Olink-SomaScan', nrow = 4, ncol = 2,
                       facet_labels = facet_labels, pval_table = olink_soma_pvals) +
  labs(caption = 'Figure legend on the next page') +
  theme(plot.caption = element_text(hjust = 0.5, size = 8, face = 'bold',
                                    margin = margin(t = 10, b = 10)))
