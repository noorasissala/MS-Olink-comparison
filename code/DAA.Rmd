...
### Overlapping proteins - no NAs (no missing values in MS, all Olink proteins)

```{r}

daa_res_noNA_overlap <- DAA(
  filter(ms_noNA, UniProt %in% overlap_no_NA),
  filter(olink_data_wide, UniProt %in% overlap_no_NA),
  test_grp = female,
  ref_grp = male,
  path = path,
...
ggsave(filename = 'replication_of_DAPs_all_overlappingProteins.pdf',
       path = path,
       width = 12,
       height = 6.5,
       units = 'cm')
...
ggsave(filename = 'replication_of_DAPs_noMissing_overlappingProteins.pdf',
       path = path,
       width = 12,
       height = 6.5,
       units = 'cm')
